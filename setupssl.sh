...
if [ -f ${prefix}cert8.db ] ; then
    ...
    if ../shared/bin/certutil -L $prefixarg -d . -n "Server-Cert-adm02" 2> /dev/null ; then
...
# 6. Generate the self-signed certificate: 
    ...
    ../shared/bin/certutil -S -n "CA certificate" -s "cn=CAcert-adm02" -x -t "CT,," -m 1000 -v 120 -d . -z noise.txt -f pwdfile.txt
...
# 7. Generate the server certificate:
    ...
    ../shared/bin/certutil -S -n "Server-Cert-adm02" -s "cn=$myhost,ou=Fedora Directory Server" -c "CA certificate" -t "u,u,u" -m 1001 -v 120 -d . -z noise.txt -f pwdfile.txt
fi

if test -n "$needASCert" ; then
# Generate the admin server certificate
    echo Creating the admin server certificate
    ../shared/bin/certutil -S -n "server-cert-adm02" -s "cn=$myhost,ou=Fedora Administration Server" -c "CA certificate" -t "u,u,u" -m 1002 -v 120 -d . -z noise.txt -f pwdfile.txt
...
# export the admin server certificate/private key for import into its key/cert db
    ...
    ../shared/bin/pk12util -d . -o adminserver.p12 -n server-cert-adm02 -w pwdfile.txt -k pwdfile.txt
    if test -n "$isroot" ; then
...
# import the admin server key/cert
    ...
    ../shared/bin/pk12util -d . -P $asprefix -n server-cert-adm02 -i adminserver.p12 -w pwdfile.txt -k pwdfile.txt
...
cn: RSA
nsSSLPersonalitySSL: Server-Cert-adm02
